...
__attribute__((fastcall)) void create_note_instr(note_instr *n, instrument *i, int f, int a){
  ...
  n->instr = i;
  //create_adsr(&(n->env),
  //            calc_freq(3.258886363e-3f,HALFTONE,i->a),
  //            calc_freq(3.258886363e-3f,HALFTONE,i->d),
  //            (float)(i->s) / 127.f,
  //            calc_freq(3.258886363e-3f,HALFTONE,i->r));
  create_adsr(&(n->env),
              (float) i->a / 127.f,
              (float) i->d / 127.f,
              (float) i->s / 127.f,
  ...
  register int os = 2; do create_osc(n->o+os, i->o[os].type, 
                        calc_freq(0.405570987f,HALFTONE,i->o[os].freqt+f) * 
...
__attribute__((fastcall)) float do_note_instr(note_instr *n){
  ...
  if(n->used){
    if((e = do_adsr(&(n->env)))<=0.f){
      ...
      n->f = 1.5f * sin4k(n->cutoff / SAMPLERATE_F / 2.0f);
